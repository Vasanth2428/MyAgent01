...
# 1. Remove phase assertions at the end
content = re.sub(r'assert "PLANNING".*?rules were not injected via context!"\n', "", content, flags=re.DOTALL)

# 2. Add architect mock to mock_build_model_with_fallback
...
content = re.sub(r'    else:\n        mock_model\.invoke\.return_value = AIMessage\(content="Mocked worker response\."\)', architect_mock + '\n        mock_model.invoke.return_value = AIMessage(content="Mocked worker response.")', content)

# 3. Replace PredefinedCodingWorker entirely using exact string matching for the start and end of the class
class_start = "class PredefinedCodingWorker:"
class_end = "supervisor_mock_instance = PredefinedSupervisor()"
...
start_i = content.find(class_start)
end_i = content.find(class_end)
content = content[:start_i] + new_coding_worker + content[end_i:]

with open(filepath, "w") as f:
    f.write(content)
print("Applied reliable patch to run_offline_pipeline.py!")
