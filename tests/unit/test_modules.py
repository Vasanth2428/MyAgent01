...
class TestCompressor(unittest.TestCase):
    ...
    def test_compress_over_budget(self):
        docs = ['This is a sentence about dogs.', 'This is a sentence about cats.', 'This is completely unrelated data.']
...
class TestCompressorSegmentation(unittest.TestCase):
    ...
    def test_compress_preserves_formatting(self):
        ...
        docs = ['Paragraph one about cats and dogs and animals in general.', 'Paragraph two about cats specifically and their behavior patterns.']
...
class TestRerankerNormalization(unittest.TestCase):
    ...
    def test_scores_bounded_zero_to_one(self, mock_get_reranker):
        ...
        def mock_rerank_impl(request):
            scores = [0.95, 0.05, 0.5, 0.1, 0.2]
            return [{'id': p['id'], 'text': p['text'], 'score': scores[i]} for (i, p) in enumerate(request.passages)]
        mock_reranker.rerank.side_effect = mock_rerank_impl
        mock_get_reranker.return_value = mock_reranker
        reranker = NeuralReranker()
        candidates = [{'text': 'doc about cats', 'score': 0.9}, {'text': 'doc about dogs', 'score': 0.8}, {'text': 'doc about birds', 'score': 0.7}, {'text': 'doc about fish', 'score': 0.6}, {'text': 'doc about mice', 'score': 0.5}]
    ...
    def test_scores_sorted_descending(self, mock_get_reranker):
        ...
        def mock_rerank_impl(request):
            scores = [0.9, 0.5, 0.1]
            results = [{'id': p['id'], 'text': p['text'], 'score': scores[i]} for (i, p) in enumerate(request.passages)]
            return sorted(results, key=lambda x: x['score'], reverse=True)
        mock_reranker.rerank.side_effect = mock_rerank_impl
        mock_get_reranker.return_value = mock_reranker
        reranker = NeuralReranker()
        candidates = [{'text': 'low relevance doc', 'score': 0.3}, {'text': 'high relevance doc', 'score': 0.9}, {'text': 'negative relevance doc', 'score': 0.1}]
...
class TestXMLContextFormatting(unittest.TestCase):
    ...
    def test_xml_tags_present(self):
        ...
        results = [{'text': 'Document about routing protocols.', 'source': 'routing.txt'}, {'text': 'Document about DNS resolution.', 'source': 'dns.txt'}]
        formatted_parts = []
        for r in results:
...
class TestGenerationResult(unittest.TestCase):
    ...
    def test_generation_result_structure(self):
        ...
        result = GenerationResult(response='test response', prompt='test prompt', token_usage={'prompt': 10, 'completion': 5, 'total': 15}, context_used_percent=12.5, grounding_score=0.85, latency_ms=150.0)
    ...
    def test_generation_result_unsupported_claims_default(self):
        ...
        from src.core.services.generation_service import GenerationResult
        result = GenerationResult(response='test', prompt='test', token_usage={'prompt': 0, 'completion': 0, 'total': 0}, context_used_percent=0.0, grounding_score=1.0, latency_ms=0.0)
        self.assertEqual(result.unsupported_claims, [])
...
class TestRetrievalServiceRRF(unittest.TestCase):
    ...
    def test_rrf_ranking_logic(self):
        ...
        def mock_retrieve(q, top_k, source_filter=None):
            if q == 'Q1':
                return ([{'text': 'docA', 'score': 0.95}, {'text': 'docB', 'score': 0.92}], 0.1, 0.2)
            elif q == 'Q2':
                return ([{'text': 'docB', 'score': 0.9}, {'text': 'docC', 'score': 0.88}], 0.1, 0.2)
            return ([], 0.0, 0.0)
