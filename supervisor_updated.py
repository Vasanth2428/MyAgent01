...
class SupervisorDecision(BaseModel):
    ...
    parallel_tasks: List[str] = Field(
        default_factory=list,
        description="If routing to coding_worker, you may provide multiple independent tasks here to be executed concurrently by separate workers."
    )
    ...
    try:
        ...
        if all_tasks_complete(plan):
            ...
            if (
                ...
                logger.warning(
                    "Supervisor attempted to reroute a validated task (%s) back to coding_worker. Overriding.",
                    selected_task.id
                )
                selected_task = get_next_pending_task([t for t in plan if t.id != selected_task.id])
            ...
            if messages and isinstance(messages[-1], AIMessage):
                ...
                if last_agent == next_agent and next_agent not in ["coding_worker", "synthesizer"]:
